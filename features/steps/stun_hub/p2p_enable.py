...
"""
    ...
    接口文档: http://10.3.0.10/root/doc/wikis/stun-hub-srv_api#3-p2p
    接口说明: stun-hub接收反向控制开启指定节点p2p功能的请求, p2p-ops -> stun-hub
    请求参数说明:
...
@given("add a valid peer_id to p2p_enable")
...
@then("there should be {count} p2p_enable in RRPC_{stun_ip}")
...
@then("rrpc_command about p2p_enable in RRPC_{expect_stun_ip} should be correct")
def verify_rrpc_is_correct(context, expect_stun_ip):
    ...
    for actual_rrpc_command in redis_value:
        ...
        assert_that(actual_rrpc_command["cmd"], equal_to("p2p_enable"), "cmd should be {0}".format("p2p_enable"))
