...
@when('channel-srv receives stopchannel request {if_with} {file_url}')
def receive_stop_channel(context, if_with, file_url):
    if if_with is "without":
        url = "/stopchannel?pid=" + str(context.peer_id)
    else:
        url = "/stopchannel?pid=" + str(context.peer_id) + "&url=" + str(file_url)
